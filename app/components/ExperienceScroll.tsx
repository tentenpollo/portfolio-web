  ...
  return (
    ...
    >
      <div className="absolute top-0 left-0 w-full z-10 px-6 md:px-12 lg:px-24 pt-24 md:pt-32">
        <p className="text-xs font-semibold tracking-[0.2em] uppercase text-outline mb-4">Career Path</p>
        <h2 className="font-h1 text-h1 text-on-surface">Experience</h2>
      </div>

      <div className="h-full w-full overflow-hidden">
        <div ref={trackRef} className="flex h-full min-w-full will-change-transform">
            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12 w-full items-center">
                <div className="md:col-span-5">
                  <p className="text-sm font-medium text-outline mb-4 tracking-wide">{exp.date}</p>
      ...
      <div className="absolute bottom-12 left-0 w-full px-6 md:px-12 lg:px-24 z-10">
        <div className="relative mx-auto max-w-3xl">
          ...
          <div
            ...
            style={{
              width: `${(activeIndex / Math.max(experiences.length - 1, 1)) * 100}%`,
            }}
          />
          <div className="relative grid grid-cols-3 gap-6">
            {experiences.map((exp, i) => (
              <div key={exp.title} className="flex flex-col items-center text-center gap-3">
                <div
                ...
                <div className="space-y-1">
                  ...
                  <p
                    className={`text-xs md:text-sm transition-colors duration-500 ${
                      i === activeIndex ? "text-on-surface-variant" : "text-on-surface-variant/60"
                    }`}
                  >
                    {exp.company}
                  </p>
